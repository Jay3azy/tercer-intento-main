...
void guardarComprasEnArchivo(char peliculas[][4][40], double precio[], char clientes[][2][40], int reserva[][4], const char* archivo) {
    FILE *file = fopen(archivo, "w");
    if (file == NULL) {
    ...
    for (int i = 0; i < 10; i++) {
        if (reserva[i][0] != -1) {
            ...
            int peliculaID = reserva[i][1];
            int tipoEntrada = reserva[i][2];
            fprintf(file, "%s|%s|%s|%d|%.2f\n", clientes[clienteID][0], peliculas[peliculaID][1], peliculas[peliculaID][2], tipoEntrada, precio[tipoEntrada]);
        }
    }
